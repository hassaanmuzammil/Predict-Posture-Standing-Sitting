...
def analyze_image(file_path, model, framework, resolution, lite):
    ...
    print("Image Height and width: ", image_height, image_width)
...
def get_angle(a, b, c):
    """
    Calculate angle between 3 points with b as the middle point.

    Args:
        a,b,c: tuple of floats
            (x,y) coordinates of the three points
            

    Returns:
       the angle ABC or CBA in degrees
    """
...
def get_posture(coordinates, show=True):
    """
    From the coordinates extracted from aboce, predict / infer the posture.
    Whether the person is standing or not

    Args:
        coordinates: list of list of tuple of (string,float,float)
        
        Body parts represented by string
        Float coordinates x,y stored with string in tuple
        Inner list represents all the body parts
        Outer list represents (coordinates) inner list for all the images / frames
            
    Returns:
       the calculated posture from angle
    """
    ...
    for coordinate in coordinates:
        ...
        left_angle = get_angle(coordinate_dict['left_hip'],coordinate_dict['left_knee'],coordinate_dict['left_ankle'])
        right_angle = get_angle(coordinate_dict['right_hip'],coordinate_dict['right_knee'],coordinate_dict['right_ankle'])
